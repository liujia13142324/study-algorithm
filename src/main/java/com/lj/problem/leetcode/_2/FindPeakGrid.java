package com.lj.problem.leetcode._2;

import org.junit.Test;

import java.util.Arrays;

/**
 * 1901. 寻找峰值 II
 * 提示
 * 一个 2D 网格中的 峰值 是指那些 严格大于 其相邻格子(上、下、左、右)的元素。
 * 给你一个 从 0 开始编号 的 m x n 矩阵 mat ，其中任意两个相邻格子的值都 不相同 。找出 任意一个 峰值 mat[i][j] 并 返回其位置 [i,j] 。
 * 你可以假设整个矩阵周边环绕着一圈值为 -1 的格子。
 * 要求必须写出时间复杂度为 O(m log(n)) 或 O(n log(m)) 的算法
 *
 *
 * 示例 1:
 * 输入: mat = [[1,4],[3,2]]
 * 输出: [0,1]
 * 解释: 3 和 4 都是峰值，所以[1,0]和[0,1]都是可接受的答案。
 *
 * 示例 2:
 * 输入: mat = [[10,20,15],[21,30,14],[7,16,32]]
 * 输出: [1,1]
 * 解释: 30 和 32 都是峰值，所以[1,1]和[2,2]都是可接受的答案。
 *
 * 提示：
 * m == mat.length
 * n == mat[i].length
 * 1 <= m, n <= 500
 * 1 <= mat[i][j] <= 105
 * 任意两个相邻元素均不相等.
 */
public class FindPeakGrid {


    @Test
    public void test2() {
        System.out.println(Arrays.toString(findPeakGrid(new int[][]{{1, 4}, {3, 2}})));
//        System.out.println(Arrays.toString(findPeakGrid(new int[][]{{70,50,40,30,20}, {100,1,2,3,4}})));
//        System.out.println(Arrays.toString(findPeakGrid(new int[][]{{1,2,3}, {8,16,4},{7,6,5}})));
//        System.out.println(Arrays.toString(findPeakGrid(new int[][]{{10,20,15}, {21,30,14},{7,16,32}})));
    }

    @Test
    public void test() {
        int[][] mat = new int[][]{{-1,-1,-1,-1}, {-1, 1, 4, -1}, {-1, 3, 2, -1}, {-1, -1, -1, -1}};
        int col = mat[0].length - 2;
//        System.out.println(mat.length);
//        System.out.println(mat[0].length);
        for (int i = 0; i < 4; i++) {
            int m = i / col + 1;
            int n = (i % col) + 1;
            System.out.println(mat[m][n]);
        }
    }

    public int[] findPeakGrid(int[][] mat) {
        int l = -1;
        int r = mat.length - 1;

        while (l + 1 < r) {
            int mid = (l + r) >>> 1;
            int maxIdx = maxIndex(mat[mid]);
            if (mat[mid][maxIdx] > mat[mid + 1][maxIdx]) {
                r = mid;
            }else {
                l = mid;
            }
        }

        return new int[]{r, maxIndex(mat[r])};
    }

    private int maxIndex(int[] array) {
        int maxIdx = 0;
        for (int i = 1; i < array.length; i++) {
            if (array[i] > array[maxIdx]) {
                maxIdx = i;
            }
        }
        return maxIdx;
    }


}
